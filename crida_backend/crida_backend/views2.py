...
def batch_predict_endpoint(request):
    ...
    if body['csvData']:
        ...
        csvData = pd.DataFrame([x.replace('\r', '').strip().split(',') for x in str(body['csvData']).split('\n')])

        csvData.columns = csvData.iloc[0]
        csvData = csvData.reindex(csvData.index.drop(0))
        X = csvData[:-1]
        print("Start xxxxxxxxxxxxxxxxx")
        print(X)
        print("End xxxxxxxxxxxxxxxxx")
        ...
        try:
            ...
            X['MONTH'] =  X['DATETIME'].apply(lambda x: toDateTime(x,"%m-%d-%Y %H:%M:%S").month)
            X['WEEKDAY'] =  X['DATETIME'].apply(lambda x: toDateTime(x,"%m-%d-%Y %H:%M:%S").weekday())
            X['PLANNED_HOUR'] =  X['DATETIME'].apply(lambda x: toDateTime(x,"%m-%d-%Y %H:%M:%S").hour)
        ...
        X_categorical = pd.DataFrame(one_hot_enc_arr.fit_transform(X[categorical_features]).toarray())
        ...
        X_categorical = pd.DataFrame(one_hot_enc_dep.fit_transform(X[categorical_features]).toarray())
